...
export class ManufacturingKitsService {
  ...
  async generate(orderId: string) {
    ...
    await this.prisma.$transaction(async (tx: any) => {
      ...
      if (!order) throw new NotFoundException('Orden de manufactura no encontrada'); if (['CANCELED', 'ON_HOLD', 'COMPLETED'].includes(order.status)) throw new ConflictException('La orden no permite preparar kits');
  ...
  private async closeKit(kitId: string, rawVersion: unknown, action: 'RELEASE' | 'CANCEL', rawText: unknown) {
    ...
    await this.prisma.$transaction(async (tx: any) => {
      ...
      if (action === 'CANCEL' && await tx.manufacturingAssemblyExecution.count({ where: { tenantId, kitId: kit.id } })) throw new ConflictException('El kit ya tiene una ejecución de ensamble y no puede cancelarse');
