...
export class ServiceOrdersService {
  ...
  private async realignPreventivePlanFutureOrders(tx: any, tenantId: string, serviceOrderId: string) {
    ...
    if (!plan) return;
    if (String(plan.pmPlanId || '') !== String(so.pmPlanId || '')) return;
    ...
    const occupiedByManual = new Set<number>(
      (futureRows ?? [])
        .filter((r: any) => !this.isAutoGeneratedPreventiveSo(r, asset.code))
        .map((r: any) => (r?.dueDate ? new Date(r.dueDate).getTime() : null))
    ...
    while (cursor.getTime() <= horizonEnd.getTime() && guard < 5000) {
      ...
      if (ts >= now.getTime() && !occupiedByManual.has(ts)) {
        candidates.push(new Date(ts));
      }
