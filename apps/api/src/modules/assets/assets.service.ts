...
export class AssetsService {
  constructor(
    ...
    private readonly carryoverService: ServiceOrderCarryoverService,
  ) {}
  ...
  private async attachPendingCarryoversToServiceOrder(
    ...
    return this.carryoverService.attachPending(tx, tenantId, destination, reason);
  }
    ...
    return this.withTenantRLS(async (tx) => {
      ...
      for (const due of candidates) {
        ...
        const wo = await tx.workOrder.create({
          ...
          select: { id: true, dueDate: true, createdAt: true, assetCode: true, formData: true },
        });
        await this.attachPendingCarryoversToServiceOrder(tx, tenantId, {
          id: wo.id,
