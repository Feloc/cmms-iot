...
export default function TelemetryTab({ deviceId }: { deviceId: string }) {
  console.log('para telemetry tab',deviceId);
  ...
  async function loadTelemetry() {
    console.log('ajecutando loadTelemetry');
    console.log(deviceId);
    
    
    if (!deviceId || !session?.user?.tenantId) return;
    console.log('Si autorizacion');
    ...
    try {
      const qs = new URLSearchParams({
        ...
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/devices/${deviceId}/telemetry?${qs.toString()}`,
            {
                headers: {
